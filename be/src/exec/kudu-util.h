...
/// Casts 'value' according to the primitive type in 'type' and writes it into 'row'
/// at position 'col'. If the primitive type is STRING or VARCHAR, 'copy_strings'
/// determines if 'value' will be copied into memory owned by the row.
/// If false, string data must remain valid while the row is being used.
Status WriteKuduValue(int col, ColumnType type, const void* value, bool copy_strings,
    kudu::KuduPartialRow* row) WARN_UNUSED_RESULT;

/// Casts 'value' according to the primitive type in 'type' and create a
/// new KuduValue containing 'value' which is returned in 'out'.
Status CreateKuduValue(
    ColumnType type, void* value, kudu::client::KuduValue** out) WARN_UNUSED_RESULT;
...
ColumnType KuduDataTypeToColumnType(kudu::client::KuduColumnSchema::DataType type,
    kudu::client::KuduColumnTypeAttributes type_attributes);
