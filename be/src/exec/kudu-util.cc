...
Status WriteKuduValue(int col, ColumnType col_type, const void* value, bool copy_strings,
...
ColumnType KuduDataTypeToColumnType(DataType type,
  ...
  switch (type) {
    ...
    case DataType::DECIMAL: return ColumnType::CreateDecimalType(
          type_attributes.precision(), type_attributes.scale());
  }
  return ColumnType(PrimitiveType::INVALID_TYPE);
}

Status CreateKuduValue(ColumnType col_type, void* value, KuduValue** out) {
  PrimitiveType type = col_type.type;
  switch (type) {
    case TYPE_VARCHAR:
    case TYPE_STRING: {
